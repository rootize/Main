function [top, pick] = nms_boxes(boxes,overlap,numpart, sort_by)
...
if nargin < 4
  sort_by = size(boxes,2);% by the last number of the column always be the value of confidence.
end
...
if isempty(boxes)
  ...
  if size(boxes,1) > 1000,
   % [foo,I] = sort(boxes(:,sort_by),'descend');
    [foo,I] = sort(boxes(:,sort_by),'descend'); % You type in the wrong place!
